...
try {
  ...
  if (process.env.VERCEL) {
    console.log('[patch-prisma] Running on Vercel, using optimized copy strategy')
    ...
    console.log('[patch-prisma] Available files in .prisma/client:', clientFiles)

    // Copiar todo o diretório .prisma/client
    fs.cpSync(prismaClientPath, prismaDestPath, { recursive: true, force: true })
    ...
    const engineFiles = clientFiles.filter(file => 
    ...
    if (engineFiles.length > 0) {
      ...
      engineFiles.forEach(file => {
        try {
          ...
          console.warn(`[patch-prisma] ⚠ Failed to copy ${file} to chunks:`, error.message)
        }
      })
    }
    ...
    console.log(`[patch-prisma] Files in destination: ${destFiles.length} files`)
  ...
  if (process.env.VERCEL) {
    console.error('[patch-prisma] This error on Vercel might cause API failures')
  }
