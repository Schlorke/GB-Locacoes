...
async function generateReport(policies, indexes, primaryKeys, foreignKeys) {
  ...
  const verificationTokensHasPK = primaryKeys.some(
    (pk) => pk.table_name === 'verificationtokens'
  )

  const totalWarnings =
    authRLSWarnings + missingIndexes + (verificationTokensHasPK ? 0 : 1)
  ...
  log(
    `\n⚠️  Warnings encontrados: ${totalWarnings}`,
  ...
  if (totalWarnings === 0) {
    ...
    if (!verificationTokensHasPK) {
      log(
        '   - Execute a migration V4 ULTRA para adicionar primary key',
        'cyan'
      )
    }
  }
