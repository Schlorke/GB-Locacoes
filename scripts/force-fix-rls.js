...
async function forceFixRLS() {
  try {
    ...
    log(
      `✅ Encontradas ${existingPolicies.length} políticas existentes`,
      'green'
    )

    // Identificar políticas que precisam ser corrigidas
    const policiesToFix = existingPolicies.filter(
      (policy) =>
        policy.qual &&
    ...
    log(
      `🔍 Identificadas ${policiesToFix.length} políticas que precisam de correção`,
      'yellow'
    )
    ...
    for (let i = 0; i < policiesToFix.length; i++) {
      ...
      try {
        log(
          `📝 Corrigindo ${i + 1}/${policiesToFix.length}: ${policy.tablename}.${policy.policyname}`,
          'cyan'
        )
