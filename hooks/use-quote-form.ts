...
export function useQuoteForm(materialsData = initialMaterials) {
  ...
  const addMaterial = useCallback(
    (materialId: number) => {
      if (!selectedMaterials.find((m) => m.id === materialId)) {
        setSelectedMaterials((prev) => [...prev, { id: materialId, quantity: 1, days: 1 }])
      }
    },
    [selectedMaterials],
  )

  const updateMaterial = useCallback((materialId: number, field: "quantity" | "days", value: number) => {
    setSelectedMaterials((prev) => prev.map((m) => (m.id === materialId ? { ...m, [field]: Math.max(1, value) } : m)))
  }, [])
  ...
  const calculateTotal = useCallback(() => {
    return selectedMaterials.reduce((total, selected) => {
      ...
      return total + (material ? material.price * selected.quantity * selected.days : 0)
    }, 0)
