...
export function useNotifications() {
  ...
  const stats: NotificationStats = {
    ...
    byType: {
      quote: notifications.filter((n) => n.type === 'quote' && !n.isRead)
        .length,
      order: notifications.filter((n) => n.type === 'order' && !n.isRead)
        .length,
      payment: notifications.filter((n) => n.type === 'payment' && !n.isRead)
        .length,
      equipment: notifications.filter(
        (n) => n.type === 'equipment' && !n.isRead
      ).length,
      system: notifications.filter((n) => n.type === 'system' && !n.isRead)
        .length,
    },
  }
