              ...
              <Table>
                ...
                <TableBody>
                  {filteredQuotes.map((quote) => {
                    const StatusIcon =
                      statusConfig[quote.status]?.icon || AlertTriangle;
          ...
          <DialogFooter className="p-6 pt-4 border-t flex-col sm:flex-row gap-2 sm:gap-0">
            ...
            {selectedQuote?.status === "pending" && (
              <>
                <Button
                  onClick={() =>
                    updateQuoteStatus(selectedQuote.id, "rejected", adminNotes)
                  }
                  disabled={isUpdating}
                  variant="destructive"
                  className="w-full sm:w-auto"
                ...
                <Button
                  onClick={() =>
                    updateQuoteStatus(selectedQuote.id, "approved", adminNotes)
                  }
                  disabled={isUpdating}
            ...
            {selectedQuote?.status === "approved" && (
              <Button
                onClick={() =>
                  updateQuoteStatus(selectedQuote.id, "completed", adminNotes)
                }
                disabled={isUpdating}
                className="bg-slate-700 text-primary-foreground hover:bg-slate-600 hover:scale-105 hover:shadow-lg transition-all duration-300 w-full sm:w-auto"
              >
